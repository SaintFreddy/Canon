...
3. **Launch surfaces required before SDK public release:** a docs site, a Discord or equivalent, at least one runnable demo (the deterministic-replay-across-primitives demo is the strongest pitch), a 1.0 API surface (not 0.0.0). These are not optional if the SDK is going to be taken seriously by anyone who isn't already watching Canon. The six-week timeline must absorb building these, not assume they're free.
