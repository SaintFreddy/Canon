...
#### CF-0063 | 2026-04-24 | Source: pkt.add-proprietary-license.v1 — Canon-repo LICENSE install + DEC-RO-04 absorption

New information:
- Plan-owner decision `canon-knowledgebase/post-reopen-decisions/condition-e-license.md` (DEC-RO-04, Option 4 Proprietary / All Rights Reserved, recorded 2026-04-24) authorizes installing a proprietary LICENSE file at the root of all 5 repos (Canon, agentic-engine, canon-apps, shared-environment, agentic-engine-history). The public-SDK aspiration phrasing previously stated in `AGENTIC_WORKFLOW.md §9 L426` is withdrawn for the 0.1.0 release posture and may be revisited at a later milestone per the revisit conditions in the decision record.
- `LICENSE` at the `SaintFreddy/Canon` repo root now carries the exact proprietary text quoted in `canon-knowledgebase/post-reopen-decisions/condition-e-license.md` ("Copyright (c) 2026 SaintFreddy. All rights reserved. … by the copyright holder."). This is the Canon-repo deliverable of `pkt.add-proprietary-license.v1`; the four satellite-repo LICENSE drops (`agentic-engine`, `canon-apps`, `shared-environment`, `agentic-engine-history`) are deferred to separate sibling issues opened against those repositories so each lands as a single-file PR with its own CI signal, per the scope choice recorded in the triggering issue body.
- Condition (e.2) is now fully absorbed into accepted Canon repo truth alongside conditions (a), (b), (c), (d), (e.1), (f), (g), (h), and (i); condition (j) Canon/packages|services|workers quarantined-draft state remains the only reopen-condition still staged for remediation via the two-packet sequence `pkt.quarantine-invariant-sweep.v1` followed by `pkt.quarantine-delete.v1`.

Impact:
- Canon's repo-level license posture is now unambiguous and matches the recorded decision. Downstream packets that reference the LICENSE file (notably `pkt.remediate-docs-truth-readme-posture.v1`, which depends on LICENSE landing first) are unblocked for the Canon repo. The four satellite LICENSE packets remain gating for the README-posture packet's runs on those repos.
- No dependency-graph node status or edge changes are required by this packet. No accepted control-plane artifact status changes; the decision absorbed here is already recorded in `canon-knowledgebase/post-reopen-decisions/` and was already surfaced as a carry-forward entry in `canon-knowledgebase/canon-knowledgebase-layer-carry-forward.md` (CF-0095). This master-plan CF entry records the Canon-repo execution of that decision.
- `AGENTIC_WORKFLOW.md` is not materialized in the `SaintFreddy/Canon` tree (Canon is control-plane + authority only; runtime-layer documents live in the runtime repos), so the §9 "public SDK posture" reconciliation step from the packet scaffold is surfaced here as deferred to whichever repo materializes `AGENTIC_WORKFLOW.md` and must be tracked by a sibling packet targeting that repo. The decision record itself (`condition-e-license.md`) already captures the withdrawal of the public-SDK aspiration, so the authority is preserved regardless of where the surface document lives.
- `CANON_PLAN_IMPACT_REPORT.md` §6 row 6 append is intentionally not performed by this packet. The droid-level execution contract and the existing master-plan "future prompts" note in CF-0061/CF-0062 both forbid edits to `CANON_PLAN_IMPACT_REPORT.md` from remediation packets, and the file is not present in this Canon-repo tree. The authority record of DEC-RO-04 landing on Option 4 is preserved in `canon-knowledgebase/post-reopen-decisions/condition-e-license.md`, in `canon-now.md`'s Recorded-decisions block, and in `canon-knowledgebase/canon-knowledgebase-layer-carry-forward.md` CF-0095, each of which is human-owned authority.

Status changes:
- No artifact-registry status change. `pkt.add-proprietary-license.v1` is a remediation packet scaffold under `docs/control-plane/implementation/packets/remediation/`; remediation packets are not registered as accepted control-plane artifacts in `artifact-registry.seed.json` (they are executable briefs, not surface or architecture artifacts), consistent with the registration pattern used by `pkt.remediate-platform-gate-truth-status.v1` and `pkt.remediate-peerdep-carveout.v1`.
- No dependency-graph edge or node status changes.

Stale items:
- None propagated by this packet. Installing a repo-root LICENSE file does not pressure any accepted spec, surface pack, blueprint, release contract, or spec-digest, and no control-plane artifact cited a specific LICENSE posture that DEC-RO-04 contradicts.

Notes for future prompts:
- Treat `canon-knowledgebase/post-reopen-decisions/condition-e-license.md` (DEC-RO-04, Option 4) as authoritative; do not re-litigate the license posture or introduce OSS / BSL phrasing without a separate recorded decision.
- Do not add SPDX headers (`SPDX-License-Identifier: ...`) to source files under this packet; such headers would contradict the proprietary posture and are explicitly forbidden by the packet scaffold. Any SPDX-header cleanup on pre-existing files is deferred to a future, separately-authorized packet.
- Satellite LICENSE PRs on `SaintFreddy/agentic-engine`, `SaintFreddy/canon-apps`, `SaintFreddy/shared-environment`, and `SaintFreddy/agentic-engine-history` must each use the same verbatim proprietary text from `condition-e-license.md` and land as single-file PRs.
- Do not edit `canon-now.md`, `canon-knowledgebase/`, `AGENTIC_ENGINE_AUDIT_LOG.md`, or `CANON_PLAN_IMPACT_REPORT.md` from remediation packets; those remain human-owned authority records.
